...
| `01.inline.3` | proven | `test_an_activity_beyond_the_declared_cost_budget_is_refused` | Every agent must have a documented cost-per-operation estimate, and operations costing ... |
| `01.inline.4` | proven | `test_empty_context_marks_the_outcome_degraded` | No agent may make a business-critical decision without access to relevant historical co... |
| `01.inline.5` | proven | `test_the_ledger_exposes_no_verb_that_undoes_an_override` | The system may never override an explicit human denial or block. |
| `01.inline.6` | proven | `test_secret_injection_requires_a_sandbox` | No agent may execute arbitrary code or commands generated by an LLM without sandboxing ... |
| `01.inline.7` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | Self-improvement may never modify the core principles, security boundaries, or human ap... |
| `01.inline.8` | proven | `test_there_are_no_circular_dependencies_between_modules` | No circular dependencies between modules. If detected, the architecture must be refacto... |
| `01.inline.9` | proven | `test_no_gateway_reaches_into_another_gateways_internals` | No module may directly invoke another module's internal functions across process bounda... |
...
| `01.inline.16` | proven | `test_context_assembly_truncates_rather_than_overflowing_the_budget` | No agent may send unfiltered raw data to an LLM without a context budget and relevance ... |
| `01.inline.17` | proven | `test_ungrounded_inference_marks_the_outcome_degraded` | No agent may present LLM-generated information as verified fact without grounding data ... |
...
| `01.inline.21` | proven | `test_an_unregistered_tool_cannot_be_invoked`, `test_registration_is_not_authorization` | No agent may invoke a tool that has not been explicitly registered, versioned, and docu... |
...
| `01.inline.25` | proven | `test_no_gateway_reaches_into_another_gateways_internals` | No module may directly query another module's database tables. |
...
| `04.34.11` | proven | `test_the_ledger_exposes_no_verb_that_undoes_an_override` | No human denial may be overridden by an agent or workflow. |
...
| `04.34.19` | proven | `test_a_definition_that_cannot_produce_a_valid_dag_is_not_registrable` | No entity may exist in an undefined or null state. |
...
| `04.34.25` | proven | `test_failure_classification_completes_within_bound_and_journals` | Failures must be classified within 60 seconds; silent failures are prohibited. |
| `04.34.26` | proven | `test_repeated_denials_raise_an_authorization_violation_incident` | Security failures trigger automatic isolation before human review. |
| `04.34.27` | proven | `test_the_circuit_breaker_trips_on_its_own_threshold` | Financial anomalies trigger automatic budget freezing before human review. |
| `04.34.28` | proven | `test_a_tripped_breaker_blocks_every_commitment` | "Fail open" is prohibited for all security, financial, and data-integrity boundaries. |
...
| `04.34.30` | proven | `test_an_adopted_entry_is_measured_to_confirmation` | All learned changes to agent behavior must be versioned, reversible, and A/B tested bef... |
...
| `04.34.33` | proven | `test_only_a_human_may_invoke_panic` | Humans must remain the ultimate decision-makers for matters involving capital, legal li... |
...
| `05.29.1` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | No runtime action may violate the constitution. |
...
| `05.29.6` | proven | `test_failure_classification_completes_within_bound_and_journals` | No failure may remain unclassified for more than 60 seconds. |
| `05.29.7` | proven | `test_a_belief_without_evidence_is_speculation` | No knowledge may be presented without source attribution and confidence scoring. |
...
| `05.29.12` | proven | `test_routine_notifications_are_batched_not_delivered_one_by_one` | Human attention is a protected resource; the runtime must minimize cognitive load, not ... |
...
| `06.23.7` | proven | `test_the_ledger_exposes_no_verb_that_undoes_an_override` | No human denial may be overridden by an agent. |
...
| `06.23.11` | proven | `test_input_failing_its_contract_is_rejected_before_the_sandbox` | No agent may proceed if a critical precondition check fails or returns ambiguous results. |
...
| `06.23.19` | proven | `test_an_adopted_entry_is_measured_to_confirmation` | All learned changes must be versioned, reversible, and A/B tested before deployment. |
...
| `07.27.5` | proven | `test_an_enabled_plugin_may_not_jump_straight_to_uninstalled` | No workflow may be destroyed while in Running or Paused state; it must transition to Co... |
...
| `07.27.8` | proven | `test_the_ledger_exposes_no_verb_that_undoes_an_override` | No human denial may be overridden by a workflow or agent. |
...
| `07.27.13` | proven | `test_a_standing_order_cannot_pre_authorize_class_d` | No workflow may make a Class D decision autonomously. |
...
| `07.27.18` | proven | `test_failure_classification_completes_within_bound_and_journals` | All workflow failures must be classified within 60 seconds; silent failures are prohibi... |
...
| `07.27.20` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | The workflow learning model may not modify constitutional principles, security boundari... |
...
| `08.25.5` | proven | `test_non_matching_event_type_is_not_routed` | No consumer may access events outside its subscribed scope and permission boundaries. |
...
| `08.25.8` | proven | `test_positions_advance_so_events_are_not_redelivered_on_poll` | No event may be delivered without at-least-once semantics. |
...
| `08.25.10` | proven | `test_composed_output_is_never_anonymous_input` | No event may contain unvalidated external data or unsanitized payloads. |
...
| `08.25.19` | proven | `test_a_broken_sink_does_not_swallow_the_incident` | Event governance anomalies must be escalated as Category 1 incidents. |
| `08.25.20` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | The Learning Model may not use event data to modify constitutional principles or securi... |
...
| `10.26.7` | proven | `test_cross_tenant_formation_is_denied` | No producer may form knowledge outside its declared scope and permission boundaries. |
| `10.26.8` | proven | `test_query_will_not_return_signals_above_the_authorized_sensitivity` | No consumer may access knowledge outside its subscribed scope, sensitivity level, and c... |
...
| `10.26.10` | proven | `test_a_hypothesis_is_invisible_to_reasoners` | No speculative or hypothetical assertion may be presented as validated knowledge to any... |
| `10.26.11` | proven | `test_a_belief_resting_on_weak_memory_is_refused` | No knowledge may be formed from unvalidated external data or unsanitized memory. |
| `10.26.12` | uncovered | — | No ontology or schema change may be deployed without backward compatibility, migration ... |
| `10.26.13` | proven | `test_the_ontology_is_not_self_modifying` | No autonomous system may modify the organizational ontology without human ratification. |
| `10.26.14` | proven | `test_audit_events_are_not_purgeable_before_seven_years` | No knowledge may be purged before its statutory retention period without Class D human ... |
| `10.26.15` | proven | `test_anonymous_memory_is_inadmissible` | No anonymous or pseudonymous knowledge formation or consumption is permitted. |
| `10.26.16` | uncovered | — | No knowledge may bypass the Knowledge Gateway for direct producer-to-consumer communica... |
| `10.26.17` | proven | `test_an_entry_without_lineage_is_rejected` | Knowledge lineage and causality chains must be preserved across all formations, derivat... |
| `10.26.18` | proven | `test_deprecation_never_deletes` | No belief may be deprecated without a justification entry linked via lineage. |
| `10.26.19` | proven | `test_a_broken_sink_does_not_swallow_the_incident` | Knowledge governance anomalies must be escalated as Category 1 incidents. |
| `10.26.20` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | The Learning Model may not use knowledge formation to modify constitutional principles,... |
...
| `11.31.13` | proven | `test_anonymous_action_is_a_violation` | No anonymous or pseudonymous decision formation or authorization is permitted. |
...
| `11.31.15` | proven | `test_quarantined_or_speculative_evidence_cannot_stand_alone` | No decision may suppress uncertainty or present speculative evidence as canonical knowl... |
...
| `11.31.18` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | No decision may override constitutional principles, security boundaries, or human sover... |
| `11.31.19` | proven | `test_failure_classification_completes_within_bound_and_journals` | No decision failure may remain unclassified or unalerted for more than 60 seconds. |
...
| `12.34.1` | proven | `test_an_unregistered_tool_cannot_be_invoked`, `test_registration_is_not_authorization` | No tool may be invoked without prior registration and validation in the Tool Registry. |
...
| `12.34.3` | proven | `test_an_invocation_outside_scope_escalates` | No tool may perform external effects outside its declared capability signature. |
| `12.34.4` | proven | `test_secret_injection_requires_a_sandbox` | No tool may execute outside its declared sandbox tier. |
...
| `12.34.6` | proven | `test_self_escalation_is_a_violation` | No tool may escalate its own permissions or access resources outside its assigned scope. |
| `12.34.7` | proven | `test_anonymous_registration_is_prohibited` | No anonymous or pseudonymous tool registration is permitted. |
| `12.34.8` | proven | `test_cross_tenant_action_is_denied` | No tool may cross tenant boundaries without explicit human approval and isolation review. |
| `12.34.9` | proven | `test_a_mutating_tool_needs_compensation` | No mutating tool may be registered without declared compensation logic. |
| `12.34.10` | proven | `test_a_cost_ceiling_breach_halts_mid_flight` | No tool invocation may exceed its cost ceiling or the consumer's remaining budget. |
...
| `12.34.12` | proven | `test_journal_is_tamper_evident` | No tool audit record may be modified, overwritten, or deleted after formation. |
| `12.34.13` | proven | `test_input_failing_its_contract_is_rejected_before_the_sandbox` | No tool may present unvalidated external data as output without schema and sanitization... |
| `12.34.14` | proven | `test_a_class_c_timeout_defers_and_never_approves`, `test_a_class_d_timeout_rejects_and_never_approves` | No approval gate for Level 3 or 4 tool invocations may auto-approve on timeout. |
...
| `12.34.19` | proven | `test_panic_completes_within_the_five_second_bound` | The Panic Protocol must halt all in-flight tool invocations within 5 seconds. |
| `12.34.20` | proven | `test_a_proposal_touching_a_non_violable_rule_is_rejected_at_validation` | No tool learning may modify sandbox requirements, security boundaries, or constitutiona... |
...
| `13.36.10` | proven | `test_a_hypothesis_is_frozen_once_formed` | No learning entry may be modified, overwritten, or deleted after validation. |
...
| `13.36.12` | proven | `test_a_human_override_of_a_learning_proposal_is_recorded` | No learning proposal may override a human sovereign decision or standing order. |
...
| `13.36.18` | proven | `test_the_journal_records_the_whole_loop_and_stays_intact` | No learning audit record may be modified, overwritten, or deleted after formation. |
| `13.36.19` | proven | `test_a_blocked_attempt_alerts_a_human_immediately_and_escalates` | Learning governance anomalies must be escalated as Category 1 incidents. |
| `13.36.20` | proven | `test_panic_completes_within_the_five_second_bound` | The Panic Protocol must halt all active learning cycles within 5 seconds of invocation. |
...
| `14.35.11` | proven | `test_an_unauthenticated_request_is_401` | No principal may bypass the Security Gateway for authentication or authorization. |
...
| `14.35.13` | proven | `test_journal_is_tamper_evident` | No security event journal may be modified, overwritten, or deleted after formation. |
...
| `14.35.18` | proven | `test_failure_classification_completes_within_bound_and_journals` | No security failure may remain unclassified or unalerted for more than 60 seconds. |
| `14.35.19` | proven | `test_security_subsystem_change_requires_human_ratification` | No security subsystem change may be deployed without human ratification. |
...
| `15.36.8` | proven | `test_contradictory_evidence_escalates_and_never_commits` | No governance artifact may proceed on unresolved contradictory evidence without human a... |
| `15.36.9` | proven | `test_class_b_rejects_a_single_option_proposal` | No governance artifact may be formed with only a single option documented for G2 and ab... |
| `15.36.10` | proven | `test_confidence_thresholds_by_authority_are_enforced` | No governance artifact may be formed without a confidence score and, for G2+, a risk as... |
...
| `15.36.12` | proven | `test_governance_reports_its_own_overhead` | No governance artifact may violate portfolio-level circuit breakers or governance overh... |
| `15.36.13` | proven | `test_an_artifact_requires_an_accountable_steward_covering_its_scope` | No anonymous or pseudonymous governance formation or ratification is permitted. |
| `15.36.14` | uncovered | — | No governance artifact may bind another tenant without explicit bilateral human ratific... |
| `15.36.15` | proven | `test_evidence_gaps_produce_an_uncertainty_rider` | No governance artifact may suppress uncertainty or present speculation as canonical evi... |
...
| `15.36.18` | proven | `test_an_artifact_contradicting_a_non_violable_rule_is_refused_at_formation` | No governance artifact may override constitutional principles, security boundaries, or ... |
| `15.36.19` | proven | `test_failure_classification_completes_within_bound_and_journals` | No governance failure may remain unclassified or unalerted for more than 60 seconds. |
| `15.36.20` | proven | `test_panic_completes_within_the_five_second_bound` | Sovereign Override must halt all active governance formations in conflict within 5 seco... |
...
| `16.35.4` | proven | `test_secret_may_not_be_written_to_a_journal_or_log` | No observability artifact may expose secret values, credentials, or authentication proofs. |
| `16.35.5` | proven | `test_the_health_report_says_it_is_not_a_compliance_ruling` | No health model may present correlation as causation without experimental validation. |
| `16.35.6` | uncovered | — | No predictive indicator may be disclosed without explicit confidence score and falsifia... |
| `16.35.7` | proven | `test_failure_classification_completes_within_bound_and_journals` | No anomaly may remain unclassified or unalerted for more than 60 seconds after detection. |
| `16.35.8` | uncovered | — | No coverage gap may remain undetected or unreported for more than 60 seconds after expe... |
| `16.35.9` | proven | `test_routine_notifications_are_batched_not_delivered_one_by_one` | No observability disclosure may overwhelm human cognitive load beyond defined budgets. |
| `16.35.10` | proven | `test_signals_are_journalled_with_their_classification` | No observability artifact may be modified, overwritten, or deleted after formation. |
| `16.35.11` | uncovered | — | No observability subsystem change may be deployed without A/B validation against baseli... |
| `16.35.12` | proven | `test_failure_classification_completes_within_bound_and_journals` | No observability failure may remain unclassified or unalerted for more than 60 seconds. |
